...
export function formItemConvertData(item, projectKey) {
    let data = {
        ...
        'enableFold':item.enableFold?1:0,
        'enableShow':item.enableShow?1:0,
        'pid':item.pid
    }
    let expand = {}
...
export function dbDataConvertForItemJson(data) {
    ...
    if (!typeMap.size > 0) {
        //根据类型获取默认数据
        _.concat(inputComponents, selectComponents,layoutComponents).forEach(item => {
    ...
    jsonItem.dataId = data.formItemId//data.id
    jsonItem.dId = data.id
    jsonItem.pid = data.pid
    jsonItem.enableFold = data.enableFold?true:false
    jsonItem.enableShow = data.enableShow?true:false
    ...
    if(data.enableFold){
        jsonItem.__config__.layout = 'collapseFormItem'
    }
