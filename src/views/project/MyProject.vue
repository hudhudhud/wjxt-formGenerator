<template>
    <div class="my-project-container">
            ...
            <el-form ref="form" label-width="100px">
                <el-form-item label="问卷更新时间">
                    ...
                    <el-input
                        ...
                        style="width: 20%; margin-left: 20px;" type="text" placeholder="请输入问卷名称"
                    />
        ...
        <div v-if="dataShowType=='gird'" class="project-grid-container">
            ...
            >
                <div v-for="p in projectList" :key="p.id" class="project-grid-item-view pointer" v-if="hasPermi('project-list')">
                    ...
                    <img class="project-grid-view-preimg"
                         src="../../assets/images/templateExample.png"
                    >
                    <p class="project-grid-view-time">创建时间：{{ p.createTime | formatDate }}</p>
                    <div class="gird-operating-btns">
                        <el-button type="text" @click="toProjectHandle(p.key,1)" v-if="hasPermi('project-edit')">
                            <i class="el-icon-edit"/>
                            编辑
                        </el-button>
                        <span>
                        <el-button type="text" @click="copyProject(p.key)" v-if="hasPermi('project-edit')">
                        ...
                        <span>
                            <el-button
                                v-if="p.status!==1&&hasPermi('project-list')" type="text" @click="toProjectHandle(p.key,5)">
                                <i class="el-icon-data-analysis"/>
                                统计
                            </el-button>
                        </span>
                        <el-popconfirm
                            v-if="p.status===2 &&hasPermi('project-edit')"
                            title="确定停止收集该问卷吗？"
            ...
            >
                ...
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        ...
                        <span>
                            <el-button
                                v-if="scope.row.status!==1 && hasPermi('project-list')"
                                type="text"
                        ...
                        <el-popconfirm
                            v-if="scope.row.status===2 && hasPermi('project-edit')"
                            title="确定停止收集该问卷吗？"
    ...
    export default {
        ...
        methods: {
            ...
            toProjectHandle(key, type) {
                this.$router.push({path: '/project/formProject', query: {key: key, active: type}})
            },
            copyProject(key) {
                this.$api.post('/user/project/copy', {'key': key}).then(res => {
                    debugger
                    if (res.flag) {
