...
export default {
    ...
    methods: {
        createProjectByTemplate() {
            ...
            let trainType = this.$route.query.trainType
            ...
            this.$api.post('/user/project/template/create/', {'key': this.projectConfig.projectKey}).then(res => {
                ...
                if (res.data) {
                    if(classId||trainType){
                        this.$api.post('/user/project/saveExpandValue', {
                            classId,
                            trainType,
                            projectKey:res.data
                        })
                    }
                    this.$router.push({path: '/project/formProject', query: {key: res.data, active: 1}})
                }
            }).catch(res => {
                console.log(res)
                this.createProjectLoading = false
