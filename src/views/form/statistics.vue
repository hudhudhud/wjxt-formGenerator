        ...
        <div class="filter-table-view">
            <el-form :inline="true" ref="filterForm" >
                ...
                <el-form-item>
                    ...
                    <el-button type="success" @click="exportProjectResult" v-if="hasPermi('project-edit')">导出</el-button>
                </el-form-item>
            </el-form>
        </div>
                    ...
                    <el-card class="box-card">
                        ...
                        <div>
                            <div v-for="item in projectItemList">
                                <h4>{{ item.label }}</h4>
                                <!--  如果是文件输入-->
                                <div v-if="item.type==17 &&activeResultRow&&activeResultRow['processData'][`${item.formItemId}`]">
                                    <el-link
                                        :href="file.url" target="_blank" type="primary"
                                        v-for="file in JSON.parse(activeResultRow['processData'][`${item.formItemId}`]['files'])">
                                        {{ file.fileName }}
                                    </el-link>
                                </div>
                                <el-tag v-else> {{
                                        activeResultRow ?
                                            activeResultRow['processData'][`${item.formItemId}`] : ''
                                    }}
                                </el-tag>
                            </div>
                        </div>
...
export default {
    ...
    data() {
        ...
        exportProjectResult() {
            ...
            this.$api_plain.get('user/project/result/export', {
        ...
        queryProjectItems() {
            this.$api.get(`/user/project/item/list`, {params: {key: this.projectKey}}).then(res => {
                if (res.data) {
                    res.data.map((item) => {
                        _.set(this.projectItemColumns, `${item.formItemId}`, item.label)
                    })
                }
