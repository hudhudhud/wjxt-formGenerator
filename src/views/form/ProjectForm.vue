        ...
        this.$api.get(url).then(res => {
            if (res.data) {
                ...
                fields.forEach(item=>{
                    //容器内children结构
                    if(item.typeId==18){
                        let children = fields.filter(it=>it.dataId!=it.pid&&it.pid==item.dataId&&!it.enableShow)
                ...
                fields.forEach(item=>{
                    if(!hasAddObj[item.dataId]){
                        if((item.pid&&!fields.find(it=>it.dataId==item.pid))||item.enableShow){
                ...
                console.log(1111,newFields)
                this.formConf.fields = newFields//fields
